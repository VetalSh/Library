package library.utils.constants;

/**
 * Constants for all pages URL
 */
public class Pages {
    private Pages() {}

    public static final String LOGIN = "/jsp/login.jsp";
    public static final String ERROR = "/jsp/error/service_error.jsp";
    public static final String HOME = "/jsp/home.jsp";
    public static final String USERS = "/jsp/admin/users.jsp";
    public static final String AUTHORS = "/jsp/admin/authors.jsp";
    public static final String MY_BOOKS = "/jsp/user/my_books.jsp";
    public static final String BASKET = "/jsp/user/basket.jsp";
    public static final String REGISTER = "/jsp/register.jsp";
    public static final String BOOKING = "/jsp/librarian/booking.jsp";
    public static final String BOOK_EDIT = "/jsp/admin/book_edit.jsp";
    public static final String AUTHOR_EDIT = "/jsp/admin/author_edit.jsp";
    public static final String XML_AUTHOR = "/WEB-INF/xml/authors.jsp";
    public static final String XML_SIMPLE_OUTPUT = "/WEB-INF/xml/simpleOutput.jsp";

    public static final String JS = "/js/libraryCustom.js";
    public static final String ROOT = "/";
}
